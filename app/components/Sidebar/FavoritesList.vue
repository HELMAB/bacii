    ...
    <div v-else :class="['space-y-1', isCompactMode ? 'space-y-0.5' : 'space-y-1']">
      ...
      >
        ...
        <div
          ...
          :class="[
            'opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-black/10 cursor-pointer',
            selectedPdf === favorite.pdf ? 'text-white' : 'text-red-500'
          ]"
          role="button"
          tabindex="0"
          @keydown.enter.stop="$emit('removeFavorite', favorite.pdf)"
          @keydown.space.prevent.stop="$emit('removeFavorite', favorite.pdf)"
